<script lang="ts">
	...
	export let variant: 'outline' | 'solid' | 'platform' | 'semisolid' | 'professional' | 'version' =
		'outline';
...
>
	...
	{#if variant === 'version'}
		<div class="confetti-container">
			{#each Array.from({ length: 6 }, (_, index) => index) as i (i)}
...
<style>
	...
	@keyframes confetti-burst {
		...
		10% {
			opacity: 1;
			transform: translate(calc(var(--tx) * 0.2), calc(var(--ty) * 0.8))
				rotate(calc(var(--tr) * 0.3)) scale(1);
		}
		100% {
			transform: translate(calc(var(--tx) * 2), calc(var(--ty) * -0.2 + 20px))
