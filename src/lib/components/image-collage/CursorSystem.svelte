<script lang="ts">
	...
	function setupPreexistingCursor(cursor: Cursor): Cursor {
		...
		const imageElement = container.querySelector(
			`.collage-image-button:nth-child(${randomImageIndex + 1})`
		);
		if (!imageElement) {
			cursor.isMovingToTarget = false;
			cursor.isStatic = true;
	...
	function updateCursorPositions() {
		...
		if (nonHumanCursors.length > maxAllowed) {
			...
			const sortedCursors = [...nonHumanCursors].sort(
		...
		cursors = cursors.map((cursor) => {
			...
			if (cursor.isMovingToTarget && !cursor.isDragging && cursor.targetImage !== null) {
				...
				const imageElement = document.querySelector(
					`${containerSelector} .collage-image-button:nth-child(${cursor.targetImage + 1})`
				);
			...
			const createCursorWithDelay = (index: number) => {
				setTimeout(
					() => {
						...
						if (!isMobile && index === 0 && Math.random() < 0.6) {
							// 60% chance for second cursor
							setTimeout(
								() => {
							...
							); // 90-120 second delay for second cursor
						}
					},
					index === 0 ? 0 : 90000 + Math.random() * 30000
				); // First cursor immediately, others after 90-120s
			};
