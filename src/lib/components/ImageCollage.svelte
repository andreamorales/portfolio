<script lang="ts">
	...
	function logState(label: string) {
		console.log(
			`[${label}] collageImages: ${collageImages.length}, imagesReady: ${imagesReady}, visibleImages: ${visibleImages.length}`
		);
	}
	...
	function generateInitialPositions(images: any[]): CollageImage[] {
		...
		console.log(
			`Generating positions with window size: ${windowWidth}x${windowHeight}, isMobile: ${isMobile}`
		);

		// Find special images first
		const womanIndex = images.findIndex(
			(img) => img.alt?.toLowerCase().includes('woman') || img.alt?.toLowerCase().includes('lady')
		);
		const knightIndex = images.findIndex(
			(img) => img.alt?.toLowerCase().includes('knight') || img.alt?.toLowerCase().includes('armor')
		);
		const flowersIndex = images.findIndex(
			(img) => img.alt?.toLowerCase().includes('flower') || img.alt?.toLowerCase().includes('bloom')
		);
		const snakeIndex = images.findIndex(
			(img) =>
				img.alt?.toLowerCase().includes('snake') || img.alt?.toLowerCase().includes('serpent')
		);
		const owlIndex = images.findIndex((img) => img.alt?.toLowerCase().includes('owl'));
		const birdIndex = images.findIndex(
			(img) => img.alt?.toLowerCase().includes('bird') || img.alt?.toLowerCase().includes('parrot')
		);
		const beetleIndex = images.findIndex(
			(img) =>
				img.alt?.toLowerCase().includes('beetle') || img.alt?.toLowerCase().includes('insect')
		);
		const rockIndex = images.findIndex(
			(img) =>
		...
		const sizedImages = images.map((img, index) => {
			...
			if (!isMobile) {
				...
				if (isOrchid || isDahlia) {
					...
					width = aspectRatio >= 1 ? windowWidth * 0.3 : windowWidth * 0.25;
				}
			}
		...
		function getOverlapPercentage(newRect: Rect, existingRect: Rect): number {
			const xOverlap = Math.max(
				0,
				Math.min(newRect.right, existingRect.right) - Math.max(newRect.left, existingRect.left)
			);
			const yOverlap = Math.max(
				0,
				Math.min(newRect.bottom, existingRect.bottom) - Math.max(newRect.top, existingRect.top)
			);
		...
		return sortedBySize.map((img, index) => {
			...
			while (attempts < maxAttempts) {
				if (isMobile) {
					...
					left = Math.max(margin, Math.min(mobileContainerWidth - img.width - margin, left));
					top = Math.max(margin, Math.min(mobileContainerHeight - img.height - margin, top));
					...
					if (index < 4) {
						...
						const bottomMargin = 50; // Bottom margin
						top =
							topMargin + Math.random() * (windowHeight - img.height - bottomMargin - topMargin);
