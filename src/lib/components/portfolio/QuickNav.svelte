<script lang="ts">
	...
	function updateActiveItem() {
		...
		const portfolioItems = sortedItems
			.map((item) => {
		...
		const activeItem = portfolioItems.find(
			(item) => item && scrollPosition >= item.top && scrollPosition <= item.bottom
		);
			...
			{#each sortedItems as item (item.id)}
				...
				>
					...
					<div class="preview-thumbnail">
						{#if item.thumbnail}
							<img src={item.thumbnail} alt={item.title} on:error={handleImageError} />
						{:else}
